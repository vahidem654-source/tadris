...
function joinClass(teacherPeerId, name){
  ...
  studentPeer.on('open', ()=>{
    ...
    const timeoutId = setTimeout(()=>{
      document.getElementById('joinStatusHint').textContent = 'بعد از ۲۰ ثانیه هنوز وصل نشد. مطمئن شوید: ۱) معلم همین الان روی «شروع کلاس» زده و تب مرورگرش باز است، ۲) از همان لینک/کد تازه استفاده می‌کنید (کد قبلی دیگر کار نمی‌کند)، ۳) اینترنت هر دو طرف وصل است.';
